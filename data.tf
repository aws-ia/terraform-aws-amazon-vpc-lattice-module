...
locals {
  ...
  global_phz = contains(keys(var.dns_configuration), "private_hosted_zone_id")
  # Obtaining a map of VPC Lattice services that require the creation of DNS configuration
  services_with_dns_config = local.global_phz ? var.services : { for k, v in var.services: k => v if contains(keys(v), "private_hosted_zone_id") }
